...
public class CrusherRecipeCategory implements IRecipeCategory<RecipeHolder<CrusherRecipe>>
{
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, RecipeHolder<CrusherRecipe> holder, IFocusGroup focuses)
    {
        ...
        for (int q = 0; q < recipe.secondaryResults().size(); q++)
        {
            ...
            builder.addSlot(RecipeIngredientRole.OUTPUT, 60 + q % 2 * 19, 80 + q / 2 * 30).addIngredient(BioIngredientTypes.STACK_WITH_CHANCE_TYPE, stack);
        }
    }
    ...
    public void draw(RecipeHolder<CrusherRecipe> holder, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY)
    {
        ...
        if (!recipe.secondaryResults().isEmpty())
            for (int q = 0; q < recipe.secondaryResults().size(); q++)
            {
                guiGraphics.blitSprite(BioSlot.FRAME, 60 + q % 2 * 19 - 1, 80 + q / 2 * 30 - 1, 18, 18);
                guiGraphics.blitSprite(BioSlot.MASK, 60 + q % 2 * 19 - 1, 80 + q / 2 * 30 - 1, 18, 18, MathHelper.ColorHelper.color(BioSlot.NORMAL_SLOT_COLOR.div(255f, new Vector4f())));
            }
