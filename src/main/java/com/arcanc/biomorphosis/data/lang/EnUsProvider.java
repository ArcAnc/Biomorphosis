...
public class EnUsProvider extends LanguageProvider
{
    ...
    {
        this.addItem(Registration.ItemReg.FLESH_PIECE, "Piece Of Flesh");
		...
		this.add(Database.GUI.GuideBook.Pages.CHAMBER.titleLangKey(), "</block;biomorphosis:multiblock_chamber/>Chamber");
	    ...
	    this.add(Database.GUI.GuideBook.Pages.FARMING.titleLangKey(), "</block;biomorphosis:meat_melon/>Farming");
	    this.add(Database.GUI.GuideBook.Pages.FARMING.textLangKey(), "Farms... Farms are the nourishment of the Swarm. We do not care for beauty. Only efficiency matters. That is why we do not raise animals. Too wasteful. Too noisy. Too slow. We grow plants instead. In your world, a melon may serve this purpose... But first, it must be changed. Softened. Taught to become meat." + recipeForBook(Registration.RecipeReg.CHAMBER_RECIPE.getRecipeType().get(), Database.rl("chamber/meat_melon_seeds_from_chamber")) + "But do not try to plant it in ordinary soil. It will not listen. It will not take root. You will need special soil - Norphed dirt. And to till it, you will need a Flesh Hoe. At first glance, it may seem unremarkable. But that is only because you are not yet truly part of the Swarm." + recipeForBook(RecipeType.CRAFTING, "_shaped", Database.rl("flesh_hoe")) + "To make the soil fertile, feed it biomass. Simply pour a bucket of it over the earth and let it seep in. One more thing, little one: till the soil from below. The roots must be invited upward. \n And remember... Do not allow too many melons to grow at once. A field that feeds the Swarm too well may begin to think it is the Swarm");
