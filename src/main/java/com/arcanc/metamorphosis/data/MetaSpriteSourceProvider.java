...
public class MetaSpriteSourceProvider extends SpriteSourceProvider
{
    public static final ResourceLocation GUI_ATLAS = ResourceLocation.withDefaultNamespace("gui");

    public MetaSpriteSourceProvider(PackOutput output, CompletableFuture<HolderLookup.Provider> lookupProvider)
    {
    ...
    protected void gather()
    {
        ...
        // TOOLTIPS
        //-------------------------------------------------------------------------------
        atlas(GUI_ATLAS).addSource(new SingleFile(Database.rl("gui/slots/frame"), Optional.of(MetaSlot.FRAME)));
        atlas(GUI_ATLAS).addSource(new SingleFile(Database.rl("gui/slots/mask"), Optional.of(MetaSlot.MASK)));
    }
