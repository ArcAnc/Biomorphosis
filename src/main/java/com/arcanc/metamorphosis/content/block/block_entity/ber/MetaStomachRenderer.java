...
import com.arcanc.metamorphosis.content.block.block_entity.MetaStomach;
...
public class MetaStomachRenderer extends GeoBlockRenderer<MetaStomach>
{
    ...
    public MetaStomachRenderer(final BlockEntityRendererProvider.Context ctx)
    {
        super(new DefaultedBlockGeoModel<>(Database.rl("stomach")));
    }

    @Override
    public void postRender(PoseStack poseStack, MetaStomach animatable, BakedGeoModel model, MultiBufferSource bufferSource, @Nullable VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int renderColor)
    {
        if (animatable == null)
            return;
        FluidSidedStorage storage = MetaStomach.getFluidHandler(animatable, null);
        if (storage == null)
            return;
    ...
    @Override
    public @Nullable RenderType getRenderType(MetaStomach animatable, ResourceLocation texture, @Nullable MultiBufferSource bufferSource, float partialTick)
    {
