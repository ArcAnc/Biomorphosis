...
public class MetaCatcherRenderer extends GeoBlockRenderer<MetaCatcher>
{
    ...
    public MetaCatcherRenderer(final BlockEntityRendererProvider.Context ctx)
    {
        super(new DefaultedBlockGeoModel<>(Database.rl("catcher")));
    }

    @Override
    public void postRender(PoseStack poseStack, MetaCatcher animatable, BakedGeoModel model, MultiBufferSource bufferSource, @Nullable VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int renderColor)
    {
        if (animatable == null)
            return;
        FluidSidedStorage storage = MetaCatcher.getFluidHandler(animatable, null);
        if (storage == null)
            return;
    ...
    @Override
    public @Nullable RenderType getRenderType(MetaCatcher animatable, ResourceLocation texture, @Nullable MultiBufferSource bufferSource, float partialTick)
    {
