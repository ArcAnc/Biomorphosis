...
package com.arcanc.metamorphosis.content.gui.container_menu;
...
public class ChestMenu extends MetaContainerMenu
{
	private final BlockPos pos;
	public final List<UUID> usingPlayers = new ArrayList<>();

	public static @NotNull ChestMenu makeServer(MenuType<?> type, int id, @NotNull Inventory playerInv, MetaChest chest)
	{
	...
	private ChestMenu(@NotNull MenuContext ctx, @NotNull Inventory playerInventory, BlockPos chestPos)
	{
		...
		{
			for (int q = 0; q < MetaChest.MAX_SLOT_AMOUNT; q++)
				this.addSlot(new MetaSlot(this, handler, q, 10 + (q % 9) * 19, 10 + (q / 9) * 19));
			this.ownSlotCount = MetaChest.MAX_SLOT_AMOUNT;
		});
	...
	private static void onOpen(final PlayerContainerEvent.@NotNull Open event)
	{
		if (event.getContainer() instanceof ChestMenu container && event.getEntity() instanceof ServerPlayer player)
		{
			...
			if (container.usingPlayers.size()  == 1)
				BlockHelper.castTileEntity(player.level(), container.getBlockPos(), MetaChest.class).
					ifPresent(MetaChest :: open);
		}
	}
